...
**1.2** You can correct these issues by explicitly setting the levels parameter in the call to `factor()`. Use, `drop = FALSE` to tell the plot not to drop unused
```{r}
gapminder %>% 
  filter(year == 1997) %>% 
...
- How many levels? What are they?
  Level", "s are essenetially categories that the data can be in. For continent there are five levels, which correspond to the five continents represented in this database: Asia, Americas, Oceania, Europe, Africa. 
- What integer is used to represent factor "Asia"?
  3 is used to represent factor "Asia". 
```{r}
gapminder$continent %>% 
  str(Asia)
...
There are 142 levels in the dataset.

```{r}
gapminder$country %>% 
  nlevels()

h_countries= c("Kenya", "Uganda", "Rwanda", "Burundi", "Tanzania")
h_gap= gapminder %>%
